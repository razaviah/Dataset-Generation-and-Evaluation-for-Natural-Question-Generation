...
def get_predictions(model, tokenizer, data_loader, num_beams=4, max_length=512, length_penalty=1):
    model.to(device)
...
def main():
    ...
    test_dataset = torch.load(args.test_file_path)
    collator = T2TDataCollator(
        tokenizer=tokenizer,
        model_type=args.model_type,
        mode="inference"
    )
    loader = torch.utils.data.DataLoader(test_dataset, batch_size=32, collate_fn=collator)
